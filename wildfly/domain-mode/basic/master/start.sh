...
then
	nohup $JBOSS_HOME/bin/domain.sh -DSERVER=$SERVER_NAME -P=$DOMAIN_BASE/$SERVER_NAME/scripts/env.properties >> $DOMAIN_BASE/$SERVER_NAME/$SERVER_NAME.out &
else
	$JBOSS_HOME/bin/domain.sh -DSERVER=$SERVER_NAME -P=$DOMAIN_BASE/$SERVER_NAME/scripts/env.properties
fi
